...
import { BIG_INT_ZERO, SUPREME, ZERO_ADDRESS } from 'src/constants'
...
export default function Manage({
  ...
  const png = SUPREME[chainId ? chainId : ChainId.AVALANCHE]
  ...
  return (
    <PageWrapper gap="lg" justify="center">
        ...
        <PoolData>
          <AutoColumn gap="sm">
            ...
            <TYPE.body fontSize={24} fontWeight={500}>
              {stakingInfo ? (
                `${stakingInfo.totalStakedInPng?.toSignificant(4, { groupSeparator: ',' })} SUPREME`
              ) : (
                <Loader />
              )}
            </TYPE.body>
        ...
        <BottomSection gap="lg" justify="center">
          <StyledDataCard bgColor={backgroundColorStakingToken} showBackground={!showGetPngButton}>
            <CardSection>
              <AutoColumn gap="md">
                <RowBetween>
                  <TYPE.white fontWeight={600}>{t('earnPage.yourStakedToken', { symbol: 'SUPREME' })}</TYPE.white>
                </RowBetween>
                <RowBetween style={{ alignItems: 'baseline' }}>
                  <TYPE.white fontSize={36} fontWeight={600}>
                    {stakingInfo?.stakedAmount?.toSignificant(6) ?? '-'}
                  </TYPE.white>
                  <TYPE.white>SUPREME</TYPE.white>
                </RowBetween>
              </AutoColumn>
            </CardSection>
          </StyledDataCard>
        ...
        {userPngUnstaked?.greaterThan('0') ? (
          <ButtonPrimary padding="10px" borderRadius="8px" width="auto" onClick={handleStakeClick}>
            {stakingInfo?.stakedAmount?.greaterThan(JSBI.BigInt(0))
              ? t('earnPage.stake')
              : t('earnPage.stakeStakingTokens', { symbol: 'SUPREME' })}
          </ButtonPrimary>
        ) : (
          <ButtonPrimary
            padding="10px"
          ...
          >
            {t('earnPage.getToken', { symbol: 'SUPREME' })}
          </ButtonPrimary>
        )}
      ...
      {userPngUnstaked?.greaterThan('0') && (
        <TYPE.main>
          {userPngUnstaked.toSignificant(6)} {t('earnPage.stakingTokensAvailable', { symbol: 'SUPREME' })}
        </TYPE.main>
      )}
    </PageWrapper>
  )
