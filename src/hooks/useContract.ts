...
import { AIRDROP_ADDRESS, BRIDGE_MIGRATOR_ADDRESS, MINICHEF_ADDRESS, SUPREME } from '../constants'
...
export function usePngContract(): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId ? SUPREME[chainId].address : undefined, PNG_ABI, true)
}
