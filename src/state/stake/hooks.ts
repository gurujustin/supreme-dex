...
export function useStakingInfo(version: number, pairToFilterBy?: Pair | null): DoubleSideStakingInfo[] {
  ...
  const png = SUPREME[chainId || ChainId.AVALANCHE]
    ...
    return rewardsAddresses.reduce<DoubleSideStakingInfo[]>((memo, rewardsAddress, index) => {
      ...
      if (
        ...
        if (
          ...
          console.error('Failed to load staking rewards info1')
          return memo
        }
...
export function useSingleSideStakingInfo(
  ...
  const supreme = SUPREME[chainId || ChainId.AVALANCHE]
  ...
  const routes = useMemo(
    (): string[][] =>
      info.map(({ conversionRouteHops, rewardToken }) => {
        return [supreme.address, ...conversionRouteHops.map(token => token.address), supreme.address]
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }),
    [info, supreme]
  )
    ...
    return rewardsAddresses.reduce<SingleSideStakingInfo[]>((memo, rewardsAddress, index) => {
      ...
      if (
        ...
        if (
          ...
          console.error('Failed to load staking rewards info2')
          return memo
        }
        ...
        const stakedAmount = new TokenAmount(supreme, JSBI.BigInt(balanceState?.result?.[0] ?? 0))
        const totalStakedAmount = new TokenAmount(supreme, JSBI.BigInt(totalSupplyStaked))
        const totalRewardRate = new TokenAmount(
          rewardToken,
          JSBI.BigInt(isPeriodFinished ? 0 : rewardRateState.result?.[0])
        )
        const earnedAmount = new TokenAmount(supreme, JSBI.BigInt(earnedAmountState?.result?.[0] ?? 0))
    ...
    return pairAddresses.reduce<any[]>((memo, pairAddress, index) => {
      ...
      if (
        ...
        if (
          ...
          console.error('Failed to load staking rewards info3')
          return memo
        }
